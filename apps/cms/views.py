...
from django.contrib.admin.views.decorators import staff_member_required#判定是否是员工
...
@method_decorator(permission_required(perm='new.change_news',login_url='/'),name='dispatch')
class NewsListView(View):
    def get(self,request):
        ...
        if start or end:
            if start:
                ...
                start_date = datetime(year=2017,month=10,day=6)
            if end:
    ...
    # around_count是当前左右可以有around_count数字然后是...
...
@method_decorator(permission_required(perm='new.add_news',login_url='/'),name='dispatch')
class WriterNewsView(View):
    def get(self,request):
...
@require_POST
@permission_required(perm='new.delete_news',login_url='/')
...
@require_GET
@permission_required(perm='new.change_newscategory',login_url='/')
def news_category(request):
    categorys = NewsCategory.objects.all()
    counts = []
    for category in categorys:
        print(category.name)
        #查找每个category中都包含有多少条新闻
        cou = News.objects.filter(category_id=category.id).count()
        counts.append(cou)
    #反转列表
...
@require_POST
@permission_required(perm='new.add_newscategory',login_url='/')
...
@require_POST
@permission_required(perm='new.delete_newscategory',login_url='/')
...
@permission_required(perm='new.add_banner',login_url='/')
def banners(request):
...
@permission_required(perm='new.delete_banner',login_url='/')
def delete_banner(request):
    banner_id = request.POST.get('banner_id')
    Banner.objects.filter(pk=banner_id).delete()
    return restful.ok()

@permission_required(perm='new.change_banner',login_url='/')
...
def person_info(request):
    return render(request,'cms/person.html')
